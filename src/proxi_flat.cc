...
std::vector<std::uint8_t> ProxiFlat::serialise() {
  ...
  insert_bytes(
      ProxiFlat::to_bytes<size_t>(static_cast<size_t>(m_is_indexed))); // Word 4
...
std::vector<std::uint8_t> ProxiFlat::to_bytes(const T value) noexcept {
   ...
   * (e.g., int, float, or POD structs) and returns its binary representation as
   * a vector of uint8_t. The bytes are laid out in memory order as per the host
...
void ProxiFlat::index_data(const std::vector<std::vector<float>> &embeddings,
   ...
   * @throws std::runtime_error If the number of features is inconsistent across
   * embeddings.
   */
...
ProxiFlat::find_indices(const std::vector<std::vector<float>> &queries) {
   ...
   * @throws std::runtime_error If a query vector has an inconsistent number of
   * features.
   */
...
std::vector<std::string> ProxiFlat::find_docs(const std::vector<float> &query) {
  /**
   * @brief Finds the documents corresponding to the K nearest neighbours for a
   * single query vector.
   *
   * @param query The query vector.
   * @return A vector of strings, where each string is a document corresponding
...
ProxiFlat::find_docs(const std::vector<std::vector<float>> &queries) {
   ...
   *
   * @param queries A vector of query vectors.
   * @return A vector of vectors, where each inner vector contains the documents
   * corresponding to the K nearest neighbours for the corresponding query.
   * @throws std::runtime_error If a query vector has an inconsistent number of
   * features.
   */
  omp_set_num_threads(m_num_threads);
...
void ProxiFlat::load(const std::string &path_str) {
  /**
   * @brief Loads a serialized ProxiFlat object from a binary file at the given
   * path.
   *
   * Format:
