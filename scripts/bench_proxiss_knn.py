...
def main():
    ...
    # 3) Build & index
    t0 = perf_counter_ns()
    p = proxiss.ProxiKNN(n_neighbours=args.k, n_jobs=args.threads, distance_function=args.objective)
    p.fit(X_train.astype(np.float32), docs_train)
    t1 = perf_counter_ns()
    index_ns = t1 - t0
    index_val, index_unit = get_unit(index_ns)

    # 4) Batched query (`predict_batch`)
    t0 = perf_counter_ns()
    idxs = p.predict_batch(X_test.astype(np.float32))
    t1 = perf_counter_ns()
    q_ns = t1 - t0
    ...
    print(f" • predict_batch total: {q_val:.3f}{q_unit} → {per_q_val:.0f}{per_q_unit}/query")
