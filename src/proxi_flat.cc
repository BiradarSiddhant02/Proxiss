...
template <typename T> std::vector<std::uint8_t> ProxiFlat::to_bytes(const T value) {
    std::vector<std::uint8_t> bytes(sizeof(T));
    std::memcpy(bytes.data(), &value, sizeof(T));
    return bytes;
}

std::vector<std::uint8_t> ProxiFlat::strings_to_bytes(const std::vector<std::string> &strs) {
...
// PRIVATE
std::vector<size_t> ProxiFlat::m_get_neighbours_memory(const std::vector<float> &query) {
...
std::vector<size_t> ProxiFlat::m_get_neighbours_disk(const std::vector<float> &query) {
    using pair = std::pair<float, size_t>;
    std::priority_queue<pair> heap;
    long long int num_samples = static_cast<long long int>(m_num_samples);
    long long int num_features = static_cast<long long int>(m_num_features);
#pragma omp for nowait
    for (size_t i = 0; i < m_num_samples; i++) {
        float distance = m_objective_function(
            std::span<const float>(query),
            std::span<const float>(m_mm_file->read_embeddings(i * m_num_features, m_num_features)));
        bool should_insert = heap.size() < m_K;
        bool should_replace = !should_insert && distance < heap.top().first;
        if (should_insert || should_replace) {
            if (should_replace)
...
std::vector<size_t> ProxiFlat::m_get_neighbours(const std::vector<float> &query) {
    if (m_in_memory) {
        return m_get_neighbours_memory(query);
    } else {
        return m_get_neighbours_disk(query);
    }
}

void ProxiFlat::in_memory_index_data(const std::vector<std::vector<float>> &embeddings,
...
void ProxiFlat::memory_mapped_index_data(const std::vector<std::vector<float>> &embeddings,
                                         const std::vector<std::string> &documents) {
    const size_t rand_string_len = 16;
    ...
    file_name.reserve(rand_string_len);
    for (size_t i = 0; i < rand_string_len; i++)
        file_name += chars[dist(gen)];
    file_name += ".bin";
    m_embeddings_file = file_name;
    std::ofstream index_file(file_name, std::ios::binary);
    if (!index_file) {
        throw std::runtime_error("Error creating file.");
    }
    for (const auto &embedding : embeddings) {
        index_file.write(reinterpret_cast<const char *>(embedding.data()),
                         embedding.size() * sizeof(float));
        if (!index_file)
            throw std::runtime_error("Failed to write embedding to file");
    }
    index_file.close();
    // --- Write document metadata to a sidecar file ---
    std::string docs_file = file_name + ".docs";
    std::ofstream doc_out(docs_file, std::ios::binary);
    if (!doc_out) {
        throw std::runtime_error("Error creating docs file.");
    }
    for (const std::string &doc : documents) {
        size_t len = doc.length();
        doc_out.write(reinterpret_cast<const char *>(&len), sizeof(size_t));
        if (len > 0)
            doc_out.write(doc.data(), len);
        if (!doc_out)
            throw std::runtime_error("Failed to write document to docs file");
    }
    doc_out.close();
    m_mm_file = std::make_unique<MemoryMappedFile>(file_name,
                                                   m_num_samples * m_num_features * sizeof(float));
    m_is_indexed = true;
}

// PUBLIC
ProxiFlat::ProxiFlat(const size_t k, const size_t num_threads, const bool in_memory,
                     const std::string objective_function)
    : m_num_samples(0), m_num_features(0), m_K(k), m_is_indexed(false), m_num_threads(num_threads),
      m_in_memory(in_memory), m_embeddings_file(""), m_objective_function_id(objective_function) {
...
void ProxiFlat::index_data(const std::vector<std::vector<float>> &embeddings,
    ...
    if (m_in_memory) {
        std::vector<float>().swap(m_embeddings_flat); // Release any memory in use
        in_memory_index_data(embeddings, documents);
    } else {
        memory_mapped_index_data(embeddings, documents);
    }
}
...
void ProxiFlat::save_state(const std::string &path_str) {
    try {
        ...
        if (!m_in_memory && m_mm_file) {
            std::filesystem::path emb_path = path / "embeddings.bin";
            std::ifstream src(m_embeddings_file, std::ios::binary);
            std::ofstream dst(emb_path, std::ios::binary);
            if (!src.is_open() || !dst.is_open())
                throw std::runtime_error("Failed to open embeddings file for copying");
            dst << src.rdbuf();
            src.close();
            dst.close();
            // --- Copy the .docs file as well ---
            std::string docs_src_path = m_embeddings_file + ".docs";
            std::filesystem::path docs_dst_path = emb_path;
            docs_dst_path += ".docs";
            if (std::filesystem::exists(docs_src_path)) {
                std::ifstream docs_src(docs_src_path, std::ios::binary);
                std::ofstream docs_dst(docs_dst_path, std::ios::binary);
                if (!docs_src.is_open() || !docs_dst.is_open())
                    throw std::runtime_error("Failed to open docs file for copying");
                docs_dst << docs_src.rdbuf();
                docs_src.close();
                docs_dst.close();
            }
        }
...
void ProxiFlat::load_state(const std::string &path_str) {
    try {
        ...
        std::filesystem::path data_file = dir_path / "data.bin";
        if (!std::filesystem::exists(data_file)) {
            throw std::runtime_error("File does not exist: " + data_file.string());
        }
        if (!std::filesystem::is_regular_file(data_file)) {
            throw std::runtime_error("Path is not a file: " + data_file.string());
        }
        std::ifstream in_file(data_file, std::ios::binary | std::ios::ate);
        if (!in_file.is_open()) {
            throw std::runtime_error("Failed to open file: " + data_file.string());
        }
        std::streamsize size = in_file.tellg();
        in_file.seekg(0, std::ios::beg);
        std::vector<std::uint8_t> buffer(size);
        if (!in_file.read(reinterpret_cast<char *>(buffer.data()), size)) {
            throw std::runtime_error("Failed to read data from file: " + data_file.string());
        }
        in_file.close();
        size_t offset = 0;
        ...
        if (!m_in_memory) {
            // Use memory-mapped file for embeddings
            std::filesystem::path emb_path = dir_path / "embeddings.bin";
            if (!std::filesystem::exists(emb_path))
                throw std::runtime_error("Embeddings file not found: " + emb_path.string());
            m_embeddings_file = emb_path.string();
            m_mm_file = std::make_unique<MemoryMappedFile>(m_embeddings_file, embeddings_size);
            // --- Load documents from sidecar file ---
            std::filesystem::path docs_path = emb_path;
            docs_path += ".docs";
            if (!std::filesystem::exists(docs_path))
                throw std::runtime_error("Documents file not found: " + docs_path.string());
            std::ifstream doc_in(docs_path, std::ios::binary);
            if (!doc_in.is_open())
                throw std::runtime_error("Failed to open documents file: " + docs_path.string());
            m_documents.clear();
            for (size_t i = 0; i < m_num_samples; ++i) {
                size_t len = 0;
                doc_in.read(reinterpret_cast<char *>(&len), sizeof(size_t));
                if (!doc_in)
                    throw std::runtime_error("Failed to read document length from docs file");
                std::string doc(len, '\0');
                if (len > 0) {
                    doc_in.read(&doc[0], len);
                    if (!doc_in)
                        throw std::runtime_error("Failed to read document content from docs file");
                }
            ...
            for (size_t len : doc_lengths) {
                ...
                m_documents.emplace_back(reinterpret_cast<const char *>(buffer.data() + offset),
                                         len);
                offset += len;
            }
        }
