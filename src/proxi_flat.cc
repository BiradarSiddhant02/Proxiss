...
std::vector<std::uint8_t> ProxiFlat::serialise() {
     ...
     * 
     * @return std::vector<std::uint8_t> A byte vector representing the full serialised state.
     * 
...
std::vector<std::uint8_t> ProxiFlat::to_bytes(const T value) noexcept {
     ...
     *
     * This templated static method takes a value of any trivially copyable type (e.g., int, float,
     * or POD structs) and returns its binary representation as a vector of uint8_t. The bytes are
     * laid out in memory order as per the host machine's endianness.
     *
     ...
     * @return std::vector<std::uint8_t> A vector containing the byte-wise representation of the input value.
     *
     * @note This function assumes native endianness. Use endian conversion if cross-platform byte order compatibility is needed.
     */
...
std::vector<std::uint8_t> ProxiFlat::strings_to_bytes(const std::vector<std::string>& strs) noexcept {
    /**
     * 
     */
...
void ProxiFlat::save(const std::string& path_str) {
    /**
     * @brief Saves the serialized Proxi object to a binary file at the given path.
     * 
     ...
     * @throws std::runtime_error if the path is invalid, inaccessible, or file writing fails.
     */
...
void ProxiFlat::load(const std::string& path_str) {
    /**
     * @brief Loads a serialized ProxiFlat object from a binary file at the given path.
     * 
     * Format:
